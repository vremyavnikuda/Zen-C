...
> **Примечание**: Используйте `{{` и `}}` для экранирования фигурных скобок внутри строк comptime.
...
| **Zig**    |    **100% (Полная)**    | Все функции                          | Нет. Использует `zig cc` как drop-in компилятор Си.      |
| **TCC**    |    **~70% (Базовая)**    | Базовый синтаксис, обобщения, трейты | Нет `__auto_type`, нет Intel ASM, нет вложенных функций. |
