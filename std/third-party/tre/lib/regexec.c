                     ...
                     int *tags, int match_eo)
{
    ...
    if (match_eo >= 0 && !(cflags & REG_NOSUB))
    {
        ...
        while (i < tnfa->num_submatches && i < nmatch)
        {
            ...
            DPRINT(("pmatch[%d] = {t%d = %d, t%d = %d}\n", i, submatch_data[i].so_tag,
                    pmatch[i].rm_so, submatch_data[i].eo_tag, pmatch[i].rm_eo));
            i++;
        }
...
static int tre_match(const tre_tnfa_t *tnfa, const void *string, ssize_t len, tre_str_type_t type,
                     size_t nmatch, regmatch_t pmatch[], int eflags)
{
    ...
    else if (tnfa->have_approx || eflags & REG_APPROX_MATCHER)
    {
        ...
        params.max_err = 0;
        params.max_cost = 0;
        status = tre_tnfa_run_approx(tnfa, string, len, type, tags, &match, params, eflags, &eo);
    }
#endif /* TRE_APPROX */
    else
    {
                 ...
                 regmatch_t pmatch[], int eflags)
{
    ...
    return tre_match(tnfa, str, (ssize_t)(len), type, nmatch, pmatch, eflags);
}

#ifdef TRE_USE_GNUC_REGEXEC_FPL
int tre_regexec(const regex_t *preg, const char *str, size_t nmatch,
                regmatch_t pmatch[_Restrict_arr_ _REGEX_NELTS(nmatch)], int eflags)
#else
int tre_regexec(const regex_t *preg, const char *str, size_t nmatch, regmatch_t pmatch[],
                int eflags)
#endif
{
                  ...
                  regmatch_t pmatch[], int eflags)
{
    ...
    return tre_match(tnfa, str, (ssize_t)(len), STR_BYTE, nmatch, pmatch, eflags);
}
...
{
    tre_tnfa_t *tnfa = (tre_tnfa_t *)preg->TRE_REGEX_T_FIELD;
    return tre_match(tnfa, str, (ssize_t)(len), STR_WIDE, nmatch, pmatch, eflags);
}

int tre_regwexec(const regex_t *preg, const wchar_t *str, size_t nmatch, regmatch_t pmatch[],
                 int eflags)
{
...
static int tre_match_approx(const tre_tnfa_t *tnfa, const void *string, ssize_t len,
                            ...
                            int eflags)
{
    ...
    if (params.max_cost == 0 && !tnfa->have_approx && !(eflags & REG_APPROX_MATCHER))
    {
        return tre_match(tnfa, string, len, type, match->nmatch, match->pmatch, eflags);
    }
                  ...
                  regaparams_t params, int eflags)
{
    ...
    return tre_match_approx(tnfa, str, (ssize_t)(len), type, match, params, eflags);
}

int tre_regaexec(const regex_t *preg, const char *str, regamatch_t *match, regaparams_t params,
                 int eflags)
{
...
int tre_regawexec(const regex_t *preg, const wchar_t *str, regamatch_t *match, regaparams_t params,
                  int eflags)
{
